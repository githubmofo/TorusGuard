...
Supported CLI flags:
- `npx torusguard harden [--target <dir>]` : Specify project root directory
- `npx torusguard harden --dry-run` : Preview candidate patches without modifying disk files
- `npx torusguard harden --severity <critical|high|medium|low>` : Filter findings by minimum severity floor
