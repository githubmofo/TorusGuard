...
Supported CLI flags:
- `npx torusguard apply [--target <dir>]` : Apply candidate patches with confirmation prompt
- `npx torusguard apply --diff` : Preview unified diffs without modifying files on disk
- `npx torusguard apply --selective` : Interactively review and approve patches one by one
- `npx torusguard apply --verify` : Trigger immediate differential AST recheck post-apply
- `npx torusguard apply --list-snapshots` : List available historical pre-apply backup snapshots
- `npx torusguard apply --yes` / `-y` : Non-interactive auto-approval mode
