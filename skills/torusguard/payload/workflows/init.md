...
Supported CLI flags:
- `npx torusguard init [--target <dir>]` : Scaffold workspace into specified root directory
- `npx torusguard init --template <golang|nextjs|fastapi|express|django|react>` : Guide stack configuration explicitly
- `npx torusguard init --audit` : Run immediate baseline audit post-initialization
- `npx torusguard init --force` : Force overwrite existing workspace and re-scaffold baseline
