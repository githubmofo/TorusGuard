...
def test_dual_track_architecture():
    ...
    assert pkg_data.get("version") in ("0.9.2", "0.9.3", "0.9.4", "0.9.5"), f"Invalid version: {pkg_data.get('version')}"
    assert "bin" in pkg_data and "torusguard" in pkg_data["bin"], "Missing bin.torusguard entry"
    print("  [PASS] package.json verified (name: torusguard, version: 0.9.5, bin configured)")
    ...
    assert any(v in res_status.stdout for v in ("0.9.2", "0.9.3", "0.9.4", "0.9.5"))
