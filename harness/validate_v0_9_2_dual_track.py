...
def test_dual_track_architecture():
    ...
    assert pkg_data.get("version") == "0.9.2", f"Invalid version: {pkg_data.get('version')}"
    assert "bin" in pkg_data and "torusguard" in pkg_data["bin"], "Missing bin.torusguard entry"
    print("  [PASS] package.json verified (name: torusguard, version: 0.9.2, bin configured)")
    ...
    assert "0.9.2" in res_status.stdout
