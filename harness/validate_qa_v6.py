...
class V6QARunner:
    ...
    def _test_phase_2_functional(self, fixtures_data: Dict[str, Any]):
        for name, data in fixtures_data.items():
            ...
            with open(run_1.manifest_file, "r", encoding="utf-8") as f:
                m1 = json.load(f)
            self.log_check("Phase 2.2", f"Manifest Schema Valid for {name}", m1.get("version") == "v6.0.0" and m1.get("target_name") == name)
    ...
    def _generate_qa_summary(self):
        qa_summary_file = PROJECT_ROOT / "QA-SUMMARY.md"
        lines = [
            ...
            f"**Final Verdict:** {'✅ READY FOR v6.0.0 RELEASE' if self.failed_count == 0 else '❌ BLOCKED'}\n",
            "---",
        ...
        # Also copy to reports dir in QA workspace
        with open(self.env.reports_dir / "QA-SUMMARY.md", "w", encoding="utf-8") as f:
            f.write("\n".join(lines) + "\n")


if __name__ == "__main__":
    qa_dir = PROJECT_ROOT / "torusguard-qa-v6"
